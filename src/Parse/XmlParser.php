<?php
	...
	class XmlParser {
		...
		protected $pathMatchStrWildcard;
		...
		public function parse(): XmlParser {
			...
			$this->pathMatchStrWildcard = null;
		...
		/**
		 * Invokes the given callback when parser points to child of an element matching given path
		 * @param string $path The path (relative to current element parser points to)
		 * @param callable $handler The callback. Will receive this parser instance as argument
		 * @param string $delimiter The path delimiter used
		 * @return XmlParser This instance
		 */
		public function eachChildOf(string $path, callable $handler, string $delimiter = '.') {
			return $this->addCallback(new ElementStartCallback(array_merge($this->parseElementPath($path, $delimiter), ['*']), $handler));
		}
		...
		protected function shouldParseElement(): bool {
			...
			return
					...
					$this->hasSubtreeCallbacks($this->pathMatchStrWildcard) ||
		...
		protected function activeCallbacks(int $depth): array {
			...
			$pathMatchStrWildcard = $depth === $this->depth ?
					$this->pathMatchStrWildcard :
					$this->makePathMatchString(array_merge(array_slice($this->pathSegments, 0, -1), ['*']));
	
	
			return $this->activeCallbacks[$depth] ??
					$this->activeCallbacks[$depth] = array_merge(
							$this->callbacks[$pathMatchStr] ?? [],
							($pathMatchStrWildcard !== null ? $this->callbacks[$pathMatchStrWildcard] ?? [] : []),
		...
		protected function parseElementPath(string $path, string $delimiter = '.'): array {
			...
			foreach ($segments as &$name) {
				...
				switch ($name[0] ?? '') {
					case '':
					case '*':
						// this check is not complete, but that's ok
						throw new InvalidArgumentException("Invalid path given: \"{$path}\"");
	
	
					case '{':
						$closingIndex = strpos($name, '}');
						if ($closingIndex === false || $closingIndex > strlen($name) - 2)
							throw new InvalidArgumentException("Invalid path given: \"{$path}\"");
						break;
		...
		protected function pushPath(string $name, string $namespaceUri = null, string $prefix = null) {
			...
			$this->pathMatchStr = $this->makePathMatchString($this->pathSegments);
			$this->pathMatchStrWildcard = $this->makePathMatchString(array_merge(array_slice($this->pathSegments, 0, -1), ['*']));
